...
function Home() {
    ...
    const [selButton, setButton]=useState();
    let buttons=[];
    for(let i=1;i<=listLen;i++)
    {
        buttons.push(<button id={i} onClick={(e)=>setButton(e.target.textContent)}>{i}</button>);
    }
    
    // let countries=countriesArr.slice((selButton*listLen)-listLen,selButton*listLen);
    console.log("listLen "+listLen);
    console.log("selected Button "+selButton)
    // console.log(selButton*listLen);
    let countries=countriesArr.slice(selButton*listLen-listLen,selButton*listLen);
  ...
  return (
    <div>
        <h3>Countries and Capitals per page</h3>
        <select onChange={(e)=>setListLen(Math.round(countriesArr.length/e.target.value))}>
